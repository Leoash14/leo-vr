...
def update_progress(progress_bar, status, pct, msg=None):
    if progress_bar:
...
def run_ffmpeg_with_progress(cmd, total_duration, progress_bar=None, status=None, offset=0.0, weight=1.0):

    if total_duration is None or total_duration <= 0:
        # fallback: run blocking, set progress to near completion at the end
        subprocess.run(cmd, check=True)
        if progress_bar:
            progress_bar.progress(min(int((offset + weight) * 100), 99))
        return
    ...
    try:
        for line in process.stderr:
            # parse ffmpeg time=00:00:01.23
            match = time_re.search(line)
            if match:
                hh, mm, ss = match.groups()
                current_time = int(hh) * 3600 + int(mm) * 60 + float(ss)
                stage_frac = min(max(current_time / total_duration, 0.0), 1.0)
                overall_frac = offset + stage_frac * weight
                pct = min(int(overall_frac * 100), 100)
                # only update if increased to reduce calls
                if progress_bar and pct != last_percent:
                    progress_bar.progress(pct)
                    last_percent = pct
                if status:
                    try:
                        status.markdown(f"Processing (ffmpeg): {pct}%")
                    except Exception:
                        pass
    finally:
        process.wait()
        if process.returncode != 0:
            raise RuntimeError(f"ffmpeg failed (rc={process.returncode}): {' '.join(cmd)}")
        # ensure we at least reflect stage completion
        if progress_bar:
            progress_bar.progress(min(int((offset + weight) * 100), 100))

def has_realsr_cli():
    return os.path.exists(r"C:\realsr-ncnn-vulkan-20220728-windows\realsr-ncnn-vulkan.exe")
def upscale_with_realsr_cli(input_path, output_path, scale=4, tile=0, gpu_id=0):

    # Prepare temp folders
    tmp_frames = tempfile.mkdtemp(prefix="realsr_frames_")
    tmp_upscaled = tempfile.mkdtemp(prefix="realsr_upscaled_")

    # 1️⃣ Extract frames from video
    subprocess.run([
        "ffmpeg", "-y", "-i", input_path,
        os.path.join(tmp_frames, "frame_%04d.png")
    ], check=True)

    # 2️⃣ Run realsr-ncnn-vulkan on the frames folder
    cmd = [
        "C:\\realsr-ncnn-vulkan-20220728-windows\\realsr-ncnn-vulkan.exe",
        "-i", tmp_frames,
        "-o", tmp_upscaled,
        "-s", "4",
        "-m", "C:\\realsr-ncnn-vulkan-20220728-windows\\models-DF2K",
        "-g", str(gpu_id)
    ]
    if tile > 0:
    ...
    try:
        ...
        print("[WARN] realsr-ncnn-vulkan failed:", e)
        return False

    # 3️⃣ Recombine upscaled frames into video
    upscaled_frames_pattern = os.path.join(tmp_upscaled, "frame_%04d.png")
    subprocess.run([
        "ffmpeg", "-y", "-r", "30", "-i", upscaled_frames_pattern,
...
def create_stereo_pair_vectorized(frame, depth):
    ...
    for y in range(h):
        ...
        if s == 0:
            ...
            # shift right for right-eye (pad left with zeros)
            right[y, s:] = frame[y, :-s]
            right[y, :s] = 0
            # left-eye: shift left (pad right with zeros)
            left[y, :-s] = frame[y, s:]
            left[y, -s:] = 0
        else:  # s < 0
            s_abs = abs(s)
            # right-eye: shift left
            right[y, :-s_abs] = frame[y, s_abs:]
            right[y, -s_abs:] = 0
            # left-eye: shift right
            left[y, s_abs:] = frame[y, :-s_abs]
            left[y, :s_abs] = 0
    return left, right
...
def inpaint_occlusions(img):
    ...
    try:
        ...
        return img
    # detect near-black holes (threshold 5)
...
# -----------------------
def combine_and_upscale(tmp_dir, fps, upscale_to, orig_audio=None, use_bicubic=True, use_realesrgan=False, progress_bar=None,                                  
                        status=None,offset=0.0, weight=1.0):
    ...
    # If user requested realsr and CLI is available, use it first
    if use_realesrgan and has_realsr_cli():
        print("[INFO] Upscaling with realsr-ncnn-vulkan (CLI)...")
        upscaled_tmp = work_side.replace(".mp4", "_upscaled.mp4")
        # compute per-axis scale as integer (work_side is side-by-side; scale in total width)
        # we'll pass scale=2 (typical); if you need different, compute target/work ratio.
        try:
            ok = upscale_with_realsr_cli(work_side, upscaled_tmp, scale=2, tile=256)
            if ok and os.path.exists(upscaled_tmp):
                # when successful, set work_side to upscaled output and continue
                work_side = upscaled_tmp
            else:
                print("[WARN] realsr CLI did not produce upscaled file; falling back to ffmpeg bicubic")
        except Exception as e:
            print("[WARN] realsr exception, falling back:", e)

    # Fallback: bicubic scale with ffmpeg
    if use_bicubic or (not use_realesrgan) or (not has_realsr_cli()):
        print("[INFO] Upscaling with ffmpeg bicubic...")
        try:
            probe = subprocess.run(
                ["ffprobe", "-v", "error", "-show_entries", "format=duration",
                 "-of", "default=noprint_wrappers=1:nokey=1", work_side],
                capture_output=True, text=True, check=True
            )
            total_duration = float(probe.stdout.strip()) if probe.stdout.strip() else None
        except Exception:
            total_duration = None
        cmd = ["ffmpeg", "-y", "-i", work_side,
               "-vf", f"scale={tw}:{th}:flags=bicubic",
               "-c:v", "libx264", "-pix_fmt", "yuv420p", final]

        run_ffmpeg_with_progress(cmd, total_duration,
                         progress_bar=progress_bar, status=status,
                         offset=0.50, weight=0.35)
    else:
        # If work_side already upscaled by realsr, copy to final
        ...
        if os.path.exists(tagged):
            try:
                ...
                if os.path.exists(tagged_foa):
                    ...
                    if progress_bar:
                       # 90–100% for final FOA/audio & metadata stage
                      update_progress(progress_bar, status, 90, "Finalizing audio & metadata...")
                      update_progress(progress_bar, status, 100, "Conversion complete ✅")
...
def convert_to_vr180(input_path, output_path=None, work_w=DEFAULT_WORK_W, work_h=DEFAULT_WORK_H,
                     max_seconds=None, fps=None, upscale=False, use_realesrgan=False, progress_bar=None, status=None):
    ...
    while True:
        ...
        if progress_bar:
         pct = int(saved / max_frames * 50)  # 0–50% for frame processing
         update_progress(progress_bar, status, pct, f"Frames processed: {pct}%")
    ...
    if progress_bar:
        ...
        if status:
            status.markdown("Finalizing: 100% ✅")
