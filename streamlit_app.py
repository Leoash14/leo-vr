...
# --- Main App UI ---
    ...
    if st.session_state.current_state == "upload":
        ...
        with col1:
            upscale_choice = st.checkbox("Upscale to 8K (slow)", value=False)
            use_realsr_choice = st.checkbox("Use realsr-ncnn-vulkan (if installed)", value=False)
            if st.button("Convert video", disabled=not st.session_state.video_path or st.session_state.is_converting, use_container_width=True):
                ...
                st.session_state.error = ""
                st.session_state.upscale_choice = upscale_choice
                st.session_state.use_realsr_choice = use_realsr_choice
                st.rerun()
        ...
        with st.spinner("Converting — this may take some time..."):
            try:
                if st.session_state.video_path:
                    base_noext = os.path.splitext(st.session_state.video_path)[0]
                    out_path = base_noext + "_vr180.mp4"

                    output_path = convert_to_vr180(
                        st.session_state.video_path,
                        out_path,
                        upscale=True if st.session_state.get("upscale_choice", False) else False,
                        use_realesrgan=True if st.session_state.get("use_realsr_choice", False) else False,
